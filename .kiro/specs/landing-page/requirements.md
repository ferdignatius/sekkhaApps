...
2. WHEN pengunjung dengan Auth_State `authenticated` mengakses route `/`, THE Landing_Page SHALL menampilkan konten publik tanpa melakukan redirect ke halaman dashboard atau halaman lain.
...
4. WHILE Auth_State sedang dalam proses resolusi, THE Landing_Page SHALL menampilkan skeleton loading state pada area Navbar CTA buttons yang mempertahankan dimensi dan posisi layout.
5. IF auth service tidak merespons dalam 3000 milliseconds sejak Landing_Page dimuat, THEN THE Landing_Page SHALL menampilkan tombol "Get started free" dan "Log in" sebagai default fallback tanpa menunggu resolusi Auth_State lebih lanjut.
...
5. THE Navbar SHALL memiliki tinggi 64px dan properti CSS `position: sticky` dengan `top: 0` serta z-index yang lebih tinggi dari seluruh konten halaman.
6. WHEN pengunjung melakukan scroll hingga posisi vertikal halaman melebihi 64px, THE Navbar SHALL menerapkan backdrop blur (`backdrop-blur-md`) pada background-nya.
7. WHEN pengunjung melakukan scroll hingga posisi vertikal halaman melebihi 0px, THE Navbar SHALL menampilkan border bawah dengan warna `{colors.hairline}`.
8. WHILE Landing_Page dirender pada Viewport_Mobile, THE Navbar SHALL mengganti navigation links horizontal dengan hamburger menu icon yang membuka drawer navigasi vertikal.
9. WHEN pengunjung menekan hamburger menu icon pada Viewport_Mobile, THE Navbar SHALL membuka drawer navigasi yang menampilkan semua navigation links dan kedua CTA buttons secara vertikal.
...
1. THE Hero_Section SHALL menampilkan satu headline utama yang terpusat secara horizontal menggunakan typography token `hero-display` (80px / weight 500 / line-height 1.05 / letter-spacing -2px) dengan warna `{colors.ink}`.
2. THE Hero_Section SHALL menampilkan satu subheadline deskriptif yang terpusat di bawah headline menggunakan typography token `subtitle` (18px / weight 400 / line-height 1.50) dengan warna `{colors.slate}`.
...
4. THE Hero_Section SHALL menampilkan tombol "See how it works" menggunakan component token `button-secondary` yang terpusat secara horizontal, berdampingan secara horizontal dengan tombol "Get started free" pada Viewport_Desktop dan Viewport_Tablet.
...
7. WHILE Landing_Page dirender pada Viewport_Mobile, THE Hero_Section SHALL menampilkan headline menggunakan typography token `heading-1` (48px / weight 500 / letter-spacing -1px) sebagai pengganti `hero-display`.
8. WHILE Landing_Page dirender pada Viewport_Mobile, THE Hero_Section SHALL menampilkan kedua CTA buttons dalam layout vertikal (stacked) dengan lebar penuh container.
...
3. THE Feature_Section SHALL menggunakan layout grid 2-kolom pada Viewport_Desktop dan Viewport_Tablet, serta 1-kolom pada Viewport_Mobile.
...
6. EACH Feature_Card SHALL menampilkan deskripsi singkat (maksimal 120 karakter) menggunakan typography token `body-md` (16px / weight 400) dengan warna `{colors.slate}`.
...
2. EACH stat item SHALL menampilkan nilai utama (angka dan satuan, contoh: "50K+" atau "99%") menggunakan typography token `stat-display` (64px / weight 500 / letter-spacing -1.5px) dengan warna `{colors.ink}`.
3. EACH stat item SHALL menampilkan label deskriptif singkat (maksimal 5 kata) menggunakan typography token `body-md` (16px / weight 400) dengan warna `{colors.slate}`.
4. THE Stats_Section SHALL menggunakan layout row horizontal dengan stat items terdistribusi merata pada Viewport_Desktop dan Viewport_Tablet.
5. WHILE Landing_Page dirender pada Viewport_Mobile, THE Stats_Section SHALL menampilkan stat items dalam layout vertikal (stacked) yang terpusat secara horizontal.
...
1. THE CTA_Banner SHALL menggunakan background warna `{colors.primary}` (`#1c1c1e`) dengan teks warna `{colors.on-primary}` (`#ffffff`) sesuai component token `cta-banner-dark`.
...
4. THE CTA_Banner SHALL menampilkan tombol "Get started free" menggunakan component token `button-on-dark` (background `{colors.on-dark}`, teks `{colors.primary}`, rounded full) yang terpusat secara horizontal.
5. WHEN pengunjung mengklik tombol "Get started free" pada CTA_Banner, THE Router SHALL menavigasi pengunjung ke halaman registrasi atau sign-up.
6. THE CTA_Banner SHALL menggunakan border radius `{rounded.feature}` (32px) pada keseluruhan container.
7. THE CTA_Banner SHALL menggunakan padding vertikal dan horizontal masing-masing sebesar `{spacing.section}` (64px).
...
5. THE Footer SHALL menampilkan teks copyright (format: "© [tahun] Sekkha. All rights reserved.") menggunakan typography token `micro` (12px / weight 500) dengan warna `{colors.on-dark-muted}`.
...
7. IF Landing_Page dirender pada Viewport_Mobile, THEN THE Footer SHALL menampilkan kolom-kolom link dalam layout vertikal (stacked) dengan setiap kolom ditampilkan penuh secara horizontal.
...
1. THE Landing_Page SHALL menggunakan container utama dengan max-width 1280px dan horizontal padding 32px pada Viewport_Desktop dan Viewport_Tablet.
2. WHILE Landing_Page dirender pada Viewport_Mobile, THE Landing_Page SHALL menggunakan horizontal padding 16px pada container utama.
3. THE Landing_Page SHALL tidak menghasilkan horizontal scrollbar pada viewport width 320px, 768px, 1024px, dan 1280px.
4. WHILE Landing_Page dirender pada Viewport_Mobile, THE Landing_Page SHALL menampilkan semua section utama dalam layout single-column sehingga tidak ada dua blok konten yang berdampingan secara horizontal.
5. EACH elemen gambar atau media dalam Landing_Page SHALL memiliki properti CSS `max-width: 100%` yang mencegah elemen tersebut overflow keluar dari container-nya pada semua breakpoint.
...
3. IF teks tombol atau link tidak secara eksplisit mendeskripsikan aksinya dalam konteks, THEN THE tombol atau link tersebut SHALL memiliki atribut `aria-label` yang mendeskripsikan aksi secara ringkas dalam bahasa yang sama dengan konten halaman.
4. WHEN elemen interaktif (tombol, link, input) menerima fokus keyboard, THE elemen tersebut SHALL menampilkan indikator fokus berupa outline atau ring yang memiliki rasio kontras minimum 3:1 terhadap warna di sekitarnya sesuai WCAG 2.1 AA.
...
7. IF gambar bersifat dekoratif dan tidak menyampaikan informasi konten, THEN THE gambar tersebut SHALL memiliki atribut `alt=""`. IF gambar menyampaikan informasi, THEN THE gambar tersebut SHALL memiliki atribut `alt` yang mendeskripsikan konten gambar tersebut.
8. THE Landing_Page SHALL memiliki rasio kontras warna minimum 4.5:1 antara teks body dan background di belakangnya untuk semua kombinasi warna teks-background yang digunakan sesuai WCAG 2.1 Level AA.
...
2. WHEN gambar berada dalam jarak 1 viewport height dari area yang terlihat, THE Landing_Page SHALL mulai mengunduh gambar tersebut — gambar yang berada di luar jarak tersebut tidak difetch hingga pengunjung melakukan scroll mendekatinya.
