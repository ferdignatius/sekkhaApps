...
export function Alert({
  ...
  return (
    <div
      role="alert"
      className={cn(
        "flex items-start gap-3 rounded-[16px] border p-4 font-sans text-sm text-[#0a0a0a] shadow-xs transition-all",
        config.container,
        className
      )}
      {...props}
    >
      <div className={cn("mt-0.5 size-5 shrink-0", config.iconClass)}>
        {icon ?? <DefaultIcon className="size-5" />}
      </div>
      <div className="flex-1 space-y-1 text-left">
        {title && (
          <p className="text-sm leading-tight font-semibold text-[#0a0a0a]">
            {title}
          </p>
        )}
        {description && (
          <div className="text-xs leading-relaxed text-[#3a3a3a]">
            {description}
          </div>
        )}
        {children}
