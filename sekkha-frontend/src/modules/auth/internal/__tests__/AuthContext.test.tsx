...
describe("Property 7: Logout Selalu Menghapus Token dan Mereset Auth_State", () => {
  ...
  it("logout always removes sekkha_access_token and resets status to unauthenticated", async () => {
...
describe("Property 6: Auth_State Terinisialisasi Benar Berdasarkan Kondisi localStorage", () => {
  ...
  it("null or empty token → unauthenticated without any fetch call", async () => {
  ...
  it("valid token + /auth/verify responds 200 → authenticated", async () => {
    ...
    await fc.assert(
      fc.asyncProperty(fc.string({ minLength: 1 }), async (token) => {
        ...
        const fetchMock = vi.fn().mockResolvedValue({
          ...
          status: 200,
          json: () =>
            Promise.resolve({ userId: "user-test", accessToken: token }),
        } as Response)
  ...
  it("valid token + /auth/verify responds 401 → unauthenticated and localStorage cleared", async () => {
  ...
  it("valid token + /auth/verify times out → unauthenticated and localStorage cleared", async () => {
