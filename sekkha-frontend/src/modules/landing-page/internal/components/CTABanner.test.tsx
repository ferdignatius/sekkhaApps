...
describe("CTABanner", () => {
  ...
  it("calls onSignUpNavigate when CTA button is clicked", () => {
    ...
    render(<CTABanner heading="Join us" onSignUpNavigate={onSignUpNavigate} />)

    fireEvent.click(
      screen.getByRole("button", {
        name: /(join for free|join now|bergabung sekarang)/i,
      })
    )
  ...
  it("still invokes onSignUpNavigate even when the function throws (no internal suppression)", () => {
    ...
    try {
      render(
        <CTABanner heading="Join us" onSignUpNavigate={throwingNavigate} />
      )
