    ...
    >
      ...
      <div className="mx-auto w-full max-w-[1280px] px-4 md:px-8">
        <div className="rounded-[24px] p-8 md:p-12 shadow-xs relative overflow-hidden bg-sekkha-surface border border-sekkha-hairline">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-0 items-center justify-center lg:divide-x lg:divide-sekkha-hairline">
            {stats.map((stat, index) => (
              <motion.div
                key={index}
                className={`flex flex-col items-center text-center ${
                  index >= 2 ? 'border-t border-sekkha-hairline pt-8 lg:border-t-0 lg:pt-0' : ''
                } ${index > 0 ? 'lg:pl-8' : ''}`}
              ...
              >
                <span className="text-4xl sm:text-5xl md:text-stat-display text-sekkha-ink font-medium tracking-[-1.5px]">
                  {stat.value}
                </span>
