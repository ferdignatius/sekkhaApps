...
export type AlertLevel =
  "normal" | "mulai_jarang" | "at_risk" | "kemungkinan_hilang" | "churned"
...
export const DUMMY_MEMBERS: MemberRecency[] = [
  {
    ...
    recentAttendances: [
      {
        eventId: "ev-8",
        title: "Puja Bhakti Mingguan",
        eventDate: "2026-07-21T09:00:00.000Z",
        scannedAt: "2026-07-21T08:50:00.000Z",
      },
      {
        eventId: "ev-9",
        title: "Puja Bhakti Mingguan",
        eventDate: "2026-07-14T09:00:00.000Z",
        scannedAt: "2026-07-14T08:53:00.000Z",
      },
      {
        eventId: "ev-10",
        title: "Puja Bhakti Mingguan",
        eventDate: "2026-07-07T09:00:00.000Z",
        scannedAt: "2026-07-07T08:58:00.000Z",
      },
    ],
  },
  {
    ...
    recentAttendances: [
      {
        eventId: "ev-11",
        title: "Meditasi Malam Minggu",
        eventDate: "2026-07-19T09:00:00.000Z",
        scannedAt: "2026-07-19T08:55:00.000Z",
      },
      {
        eventId: "ev-12",
        title: "Meditasi Malam Minggu",
        eventDate: "2026-07-05T09:00:00.000Z",
        scannedAt: "2026-07-05T08:48:00.000Z",
      },
    ],
  },
  {
    ...
    recentAttendances: [
      {
        eventId: "ev-1",
        title: "Puja Bhakti Mingguan",
        eventDate: "2026-07-08T09:00:00.000Z",
        scannedAt: "2026-07-08T08:55:00.000Z",
      },
      {
        eventId: "ev-2",
        title: "Puja Bhakti Mingguan",
        eventDate: "2026-07-01T09:00:00.000Z",
        scannedAt: "2026-07-01T08:50:00.000Z",
      },
    ],
  },
  {
    ...
    recentAttendances: [
      {
        ...
        scannedAt: "2026-07-06T08:48:00.000Z",
      },
    ],
  },
  {
    ...
    recentAttendances: [
      {
        eventId: "ev-4",
        title: "Puja Bhakti Mingguan",
        eventDate: "2026-06-30T09:00:00.000Z",
        scannedAt: "2026-06-30T08:52:00.000Z",
      },
    ],
  },
  {
    ...
    recentAttendances: [
      {
        eventId: "ev-5",
        title: "Puja Bhakti Mingguan",
        eventDate: "2026-06-27T09:00:00.000Z",
        scannedAt: "2026-06-27T08:45:00.000Z",
      },
    ],
  },
  {
    ...
    recentAttendances: [
      {
        eventId: "ev-6",
        title: "Puja Bhakti Mingguan",
        eventDate: "2026-06-16T09:00:00.000Z",
        scannedAt: "2026-06-16T08:58:00.000Z",
      },
    ],
  },
  {
    ...
    recentAttendances: [
      {
        eventId: "ev-7",
        title: "Hari Raya Asadha",
        eventDate: "2026-05-13T09:00:00.000Z",
        scannedAt: "2026-05-13T08:45:00.000Z",
      },
    ],
  },
  {
    ...
    recentAttendances: [
      {
        eventId: "ev-13",
        title: "Puja Bhakti Mingguan",
        eventDate: "2026-07-20T09:00:00.000Z",
        scannedAt: "2026-07-20T08:45:00.000Z",
      },
    ],
  },
  {
...
export function getDummyRecencyResponse(params?: {
  ...
  if (params?.level && params.level !== "all") {
    if (params.level === "warning") {
      ...
      list = list.filter(
        (m) => m.level === "kemungkinan_hilang" || m.level === "churned"
      )
  ...
  return {
    summary: {
      ...
      warningCount: DUMMY_MEMBERS.filter((m) => m.level === "mulai_jarang")
        .length,
      atRiskCount: DUMMY_MEMBERS.filter((m) => m.level === "at_risk").length,
      lostCount: DUMMY_MEMBERS.filter(
        (m) => m.level === "kemungkinan_hilang" || m.level === "churned"
      ).length,
...
export async function fetchRecencyAlerts(params?: {
  ...
  try {
    ...
    const res = await api.get<RecencyAlertsResponse>(
      `/pengurus/recency-alerts?${query.toString()}`
    )
    return res
  } catch (err) {
    console.warn(
      "API /pengurus/recency-alerts error or endpoint offline, using dummy fallback:",
      err
    )
...
export async function fetchMemberRecencyDetail(
  userId: string
): Promise<MemberDetailResponse> {
  try {
    const res = await api.get<MemberDetailResponse>(
      `/pengurus/recency-alerts/${userId}`
    )
    return res
  } catch (err) {
    console.warn(
      `API /pengurus/recency-alerts/${userId} error, using dummy fallback:`,
      err
    )
    const member =
      DUMMY_MEMBERS.find((m) => m.userId === userId) || DUMMY_MEMBERS[0]
    return {
      member,
      eventTimeline: [
        {
          ...
          title: "Puja Bhakti Mingguan",
          eventDate: "2026-07-21T09:00:00.000Z",
          attended: member.consecutiveMissedEvents === 0,
        },
        {
          eventId: "ev-2",
          title: "Puja Bhakti Mingguan",
          eventDate: "2026-07-14T09:00:00.000Z",
          attended: member.consecutiveMissedEvents <= 1,
        },
        {
          eventId: "ev-3",
          title: "Puja Bhakti Mingguan",
          eventDate: "2026-07-07T09:00:00.000Z",
          attended: member.consecutiveMissedEvents <= 2,
        },
        {
          eventId: "ev-4",
          title: "Puja Bhakti Mingguan",
          eventDate: "2026-06-30T09:00:00.000Z",
          attended: member.consecutiveMissedEvents <= 3,
        },
      ],
    }
  }
